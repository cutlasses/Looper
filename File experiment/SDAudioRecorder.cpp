...
SD_AUDIO_RECORDER::SD_AUDIO_RECORDER() :
  ...
  m_mode( MODE::STOP ),
  m_recorded_audio_file(nullptr),
  m_play_back_audio_file(nullptr),
...
void  SD_AUDIO_RECORDER::setup()
{
  auto load_file =[](File& file, const char* filename)
  {
  ...
  __disable_irq();
  load_file( m_file_1, RECORDING_FILENAME1 );
  load_file( m_file_2, RECORDING_FILENAME2 );
  __enable_irq();

  m_play_back_audio_file  = &m_file_1;
  m_recorded_audio_file   = &m_file_2;
}
...
void SD_AUDIO_RECORDER::switch_play_record_buffers()
{
  ...
  swap( m_play_back_audio_file, m_recorded_audio_file );
